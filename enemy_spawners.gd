		...
		if (len(randomEnemy) > 0):
			...
			match enemyName:
				...
				"Splasher":
					...
					enemy.global_position = activatedspawner.global_position
				"Illusioner":
					var illusioner = preload("res://illusioner.tscn").instantiate()
					get_parent().get_node("Enemies").add_child(illusioner)
					addstat(illusioner)
					illusioner.global_position = activatedspawner.global_position
